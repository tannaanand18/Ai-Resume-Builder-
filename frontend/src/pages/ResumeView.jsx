  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "Inter, sans-serif" }}>
      <style>{`
        ...
        @media (max-width: 600px) { .btn-label { display: none; } .resume-header { padding: 24px 20px !important; } .resume-body { padding: 24px 20px !important; } }
        @media print {
            ...
            {showShareMenu && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", minWidth: 220, zIndex: 200, overflow: "hidden", animation: "fadeIn 0.15s ease" }}>
                ...
                <div className="share-opt" onClick={() => handleShare("email")} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", cursor: "pointer" }}>
                  ...
                  <div><div style={{ fontSize: 13, fontWeight: 600, color: "#0f172a" }}>Email</div><div style={{ fontSize: 11, color: "#94a3b8" }}>Send via Email / Gmail</div></div>
                </div>
                ...
                <div className="share-opt" onClick={handleDownloadPDF} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px 12px", cursor: "pointer" }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: "#f0fdf4", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {downloading
                      ? <div style={{ width: 14, height: 14, border: "2px solid #dcfce7", borderTopColor: "#059669", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      ...
      <div style={{ maxWidth: 900, margin: "32px auto", padding: "0 16px 64px" }}>
        <div className="resume-page" style={{ background: "#fff", borderRadius: 12, boxShadow: "0 4px 24px rgba(0,0,0,0.08)", overflow: "hidden" }}>

          <div className="resume-header" style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", padding: "36px 48px", color: "#fff" }}>
            <h1 style={{ fontSize: 32, fontWeight: 800, margin: "0 0 4px", letterSpacing: "-0.02em" }}>{resume.full_name || "No Name"}</h1>
            {resume.professional_title && <p style={{ fontSize: 15, opacity: 0.9, margin: "0 0 12px" }}>{resume.professional_title}</p>}
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 16px", fontSize: 12, opacity: 0.85 }}>
          ...
          <div className="resume-body" style={{ padding: "32px 48px" }}>

            {resume.summary && (
              <Section title="Professional Summary">
                <p style={{ color: "#475569", lineHeight: 1.6, margin: 0, fontSize: 13 }}>{resume.summary}</p>
              </Section>
            )}
                ...
                {experiences.map((exp, i) => (
                  <div key={i} style={{ marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
                      <div><span style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>{exp.role}</span><span style={{ color: "#6366f1", fontWeight: 600, fontSize: 13 }}> @ {exp.company}</span></div>
                      <span style={{ fontSize: 12, color: "#94a3b8" }}>{exp.start_date} – {exp.end_date || "Present"}</span>
                    </div>
                    {exp.description && <p style={{ color: "#475569", lineHeight: 1.5, margin: "6px 0 0", paddingLeft: 10, borderLeft: "2px solid #e2e8f0", fontSize: 13 }}>{exp.description}</p>}
                  </div>
                ))}
              </Section>
            )}
              ...
              <Section title="Education">
                {educations.map((edu, i) => (
                  <div key={i} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
                      <div><span style={{ fontSize: 14, fontWeight: 700, color: "#0f172a" }}>{edu.degree}</span><span style={{ color: "#64748b", fontSize: 13 }}> – {edu.institution}</span></div>
                      <span style={{ fontSize: 12, color: "#94a3b8" }}>{edu.start_year} – {edu.end_year}</span>
                    </div>
                    {edu.score && <p style={{ color: "#64748b", fontSize: 12, margin: "2px 0 0" }}>Score: {edu.score}</p>}
                  </div>
                ))}
              </Section>
            )}
            ...
            {skills?.length > 0 && (
              <Section title="Skills">
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {skills.map((s, i) => (
                    <span key={i} style={{ background: "#eef2ff", color: "#4f46e5", border: "1px solid #c7d2fe", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 500 }}>
                ...
                {projects.map((p, i) => (
                  <div key={i} style={{ marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 6 }}>
                      ...
                      {p.link && <a href={p.link} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#6366f1", fontWeight: 600 }}>View →</a>}
                    </div>
                    {p.tech_stack && <p style={{ fontSize: 12, color: "#6366f1", margin: "2px 0", fontStyle: "italic" }}>{p.tech_stack}</p>}
                    {p.description && <p style={{ color: "#475569", lineHeight: 1.5, margin: "4px 0 0", fontSize: 13 }}>{p.description}</p>}
                  </div>
                ))}
              </Section>
            )}
            ...
            {certs?.length > 0 && (
              <Section title="Certifications">
                {certs.map((c, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < certs.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                    <span style={{ fontWeight: 600, color: "#0f172a", fontSize: 13 }}>{c.name}</span>
                    <span style={{ fontSize: 12, color: "#94a3b8" }}>{c.issuer}{c.issue_date ? ` · ${c.issue_date}` : ""}</span>
                  </div>
                ))}
              </Section>
            )}
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
        <h2 style={{ fontSize: 15, fontWeight: 800, color: "#0f172a", margin: 0, textTransform: "uppercase", letterSpacing: "0.05em" }}>{title}</h2>
